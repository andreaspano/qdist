...
# Some theory at the basis of  package `qdist`
...
Along with the environment where the function was created, functions interact with several other environments. The _evaluation environment_ of a function is on eof them.
...
* modify the formals of `probability()` so that it has the same formals as `pnorm(0)` plus `L` and `U` corresponding to the lower and upper threshold for truncation
* Returns `probability()`
